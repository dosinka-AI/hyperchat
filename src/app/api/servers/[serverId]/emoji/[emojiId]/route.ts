...
type Params = { params: Promise<{ serverId: string; emojiId: string }> }
...
export async function DELETE(_req: NextRequest, { params }: Params) {
  ...
  try {
    ...
    if (!hasPerm(ctx.perms, PERM.MANAGE_SERVER)) {
      return forbidden('Only moderators with the Manage Server permission can remove emoji.')
    }

    const emoji = await db.serverEmoji.findUnique({ where: { id: emojiId } })
    if (!emoji || emoji.serverId !== serverId) return notFound('Emoji not found.')
