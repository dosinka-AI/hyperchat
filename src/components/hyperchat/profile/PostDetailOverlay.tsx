...
export function PostDetailOverlay({ postId, meId, onClose, onDeleted, onOpenProfile }: Props) {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setHasMoreComments(res.post.commentCount > res.comments.length)
    } catch {
  ...
  async function toggleLike() {
    ...
    setPost({ ...post, likedByMe: nextLiked, likeCount: Math.max(0, nextCount) })
    setLikeKey((k) => k + 1)
    setCountKey((k) => k + 1)
    try {
  ...
  async function loadMoreComments() {
    ...
    setLoadingMore(true)
    try {
      const res = await apiClient.profilePostComments(post.id, pagesLoaded * PAGE)
      setComments((cur) => [...res.comments.slice().reverse(), ...cur])
      setPagesLoaded((p) => p + 1)
      setHasMoreComments(res.nextOffset !== null)
    } catch {
            ...
            <div className="px-4 pt-3 shrink-0">
              <div className="flex items-center gap-3">
                ...
                >
                  ...
                  />
                  <span key={countKey} className="text-sm font-bold tabular-nums hc-count-bump">
                    {post.likeCount}
                  </span>
                </button>
